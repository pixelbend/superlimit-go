...
type Options struct {
	...
	// By default, it is set to "LEAKY_LIMIT", but it can be customized to avoid key collisions
	KeyPrefix string
}

// DefaultOptions returns an Options struct populated with the default settings
// for the Limiter. Currently, this includes setting the `KeyPrefix` to "LEAKY_LIMIT".
//
// Defaults:
//
//   - KeyPrefix: The prefix used for keys managed by the limiter. Default is set to "LEAKY_LIMIT".
...
//     This can be created using helper functions such as `surgelimit.LimitPerSecond`, `surgelimit.LimitPerMinute`, or `surgelimit.LimitPerHour`.
//
// Returns:
...
//	limit := surgelimit.LimitPerSecond(5)
//
...
//
// The `surgelimit.Limit` struct can be easily created using helper functions like `surgelimit.LimitPerSecond`,
// `surgelimit.LimitPerMinute`, and `surgelimit.LimitPerHour`, which define the rate limit in terms of requests
...
//
//	// Create a rate limit of 5 requests per second
//	limit := surgelimit.LimitPerSecond(10)
//
...
//	limit := surgelimit.LimitPerMinute(10)
//
